...
def _log(msg):
    print("[MarkdownPreviewPro] %s" % msg)
    _file_log(msg)
...
def _preview_alive():
    ...
    if config.get("use_local_server", True) and not SERVER.running:
        # Stale flag after idle-stop or crash — recover.
...
class MarkdownPreviewProToggleCommand(sublime_plugin.WindowCommand):
    def run(self):
        ...
        if view is None:
            ...
            self.window.status_message("MarkdownPreviewPro: no active view")
            return

        if _preview_alive():
            # Already running: re-render and jump to the existing browser tab.
            _log("toggle: preview already open → focus tab + refresh")
            MarkdownPreviewProListener.render_view(
                view, force=True, open_browser=False)
            if _focus_preview_browser():
                self.window.status_message(
                    "MarkdownPreviewPro: focused preview (%s)" % _preview_url())
            else:
                self.window.status_message(
                    "MarkdownPreviewPro: could not focus browser; try Close then Toggle")
            return

        _log("toggle: opening preview")
        self.window.status_message("MarkdownPreviewPro: opening preview…")
        MarkdownPreviewProListener.render_view(view, force=True, open_browser=True)


class MarkdownPreviewProCloseCommand(sublime_plugin.WindowCommand):
    def run(self):
        _close_preview_ui(stop_server=True)
        self.window.status_message("MarkdownPreviewPro: preview closed (server stopped)")
...
class MarkdownPreviewProExportHtmlCommand(sublime_plugin.WindowCommand):
    def run(self):
        view = self.window.active_view()
        if view is None:
            return
...
class MarkdownPreviewProExportPdfCommand(sublime_plugin.WindowCommand):
    def run(self):
        view = self.window.active_view()
        if view is None:
            return
        ...
        def on_done(path):
            ...
            text = view.substr(sublime.Region(0, view.size()))
            rs = _render_settings()
            self.window.status_message("MarkdownPreviewPro: exporting PDF…")
...
class MarkdownPreviewProListener(sublime_plugin.EventListener):
    ...
    def render_view(cls, view, force=False, open_browser=False):
        ...
        mermaid_theme = view.settings().get(
            "markdown_preview_pro.mermaid_theme", rs["mermaid_theme"])
